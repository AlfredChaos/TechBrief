...
- [ ] 公开站点可访问：首页、列表页、详情页、Archive、About、Privacy、Terms 均可正常浏览
- [ ] 邮件订阅链路可用：用户可直接订阅成功、退订，并收到每日汇总邮件
- [ ] 后台管理可用：可查看来源、订阅者、内容、工作流运行情况，并可手工发布
- [ ] 微信公众号手动发布可用：支持基于站内已发布内容或手工输入生成草稿
...
- [ ] canonical_url / source_id / 平台唯一标识去重生效
...
- [ ] 详情页支持中文单栏与中英双语双栏两种阅读模式
- [ ] 首页、列表页、静态页等运营文案支持中英文切换，且采用双字段内容配置
- [ ] 后台可将待审核内容手动发布到公开站点
- [ ] 文末固定区块模板正确出现
- [ ] 公开站点支持响应式布局、Light / Dark mode、阅读进度条与悬浮订阅入口

### 2.6 邮箱订阅与每日汇总

- [ ] 支持邮箱订阅，提交后直接订阅成功
- [ ] 支持退订
- [ ] 记录订阅状态、订阅时间、退订状态和来源页面
- [ ] 新内容发布后按每日汇总策略发送邮件
...
- [ ] 仪表盘可查看当日新增、已发布、失败、订阅新增、工作流成功率、最近动态、系统状态
- [ ] 来源管理支持查看启停状态、巡检配置与发现入口
- [ ] 订阅者列表页可查看邮箱、状态、订阅时间、退订状态
- [ ] 内容列表页可查看原文、译文、状态、来源、内容类型、发布时间与可执行动作
- [ ] 内容详情页可查看原文 / 译文 / 产物 / 运行记录 / 预览
- [ ] 每日工作流执行页可查看 discover / fetch / extract / transcribe / translate / research / publish / notify 状态
- [ ] 支持从失败点重试
- [ ] 支持手动录入其他文章链接并自动抓取
